...
@synthesize isTesting, adExtras, location;

@synthesize bannerIsVisible, bannerIsInitialized;

@synthesize bannerShow, autoShow, autoShowBanner, autoShowInterstitial;

#define DEFAULT_BANNER_ID    @"ca-app-pub-6611808811429075/8320191743"

#define DEFAULT_INTERSTITIAL_ID @"ca-app-pub-6611808811429075/9796924940"
...

{
    ...
    dict = [options objectForKey:OPT_LOCATION];
    if (dict) {
        location = dict;
    }
}
...

{
    ...
    if (self.location) {
        NSNumber *latitude = [self.location objectForKey:@"latitude"];
        NSNumber *longitude = [self.location objectForKey:@"longitude"];
        NSNumber *accuracy = [self.location objectForKey:@"accuracy"];
        if (latitude && longitude && accuracy) {
            NSLog(@"Setting location for targetting, latitude=%f, longitude=%f, accurary=%f",
                  latitude.floatValue, longitude.floatValue, accuracy.floatValue);
            [request setLocationWithLatitude:latitude.floatValue
...
- (NSString *)getAdNetworkName:(NSString *)adClassName {
...
- (void)adViewDidReceiveAd:(GADBannerView *)adView {
    ...
    NSString *adClassName = [adView adNetworkClassName];
    NSString *network = [self getAdNetworkName:adClassName];
    NSString* jsonData = [NSString stringWithFormat:@"{ 'network': '%@' }", network];
    [self fireEvent:@"" event:@"onReceiveAd" withData:jsonData];
}

- (void)adView:(GADBannerView *)view didFailToReceiveAdWithError:(GADRequestError *)error {
    NSString *adClassName = [view adNetworkClassName];
    NSString *network = [self getAdNetworkName:adClassName];
    NSString* jsonData = [NSString stringWithFormat:@"{ 'error': '%@', 'network': '%@' }",
...
- (void)adViewWillPresentScreen:(GADBannerView *)adView {
    ...
    NSString *network = [self getAdNetworkName:adClassName];
    NSString* jsonData = [NSString stringWithFormat:@"{ 'network': '%@' }", network];
    [self fireEvent:@"" event:@"onPresentAd" withData:jsonData];
}
...
- (void)adViewDidDismissScreen:(GADBannerView *)adView {
    ...
    NSString *network = [self getAdNetworkName:adClassName];
    NSString* jsonData = [NSString stringWithFormat:@"{ 'network': '%@' }", network];
    [self fireEvent:@"" event:@"onDismissAd" withData:jsonData];
}

- (void)interstitialDidReceiveAd:(GADInterstitial *)interstitial {
    NSString *adClassName = [interstitial adNetworkClassName];
    NSString *network = [self getAdNetworkName:adClassName];
    NSString* jsonData = [NSString stringWithFormat:@"{ 'network': '%@' }", network];
    [self fireEvent:@"" event:@"onReceiveInterstitialAd" withData:jsonData];
...
- (void)interstitial:(GADInterstitial *)ad didFailToReceiveAdWithError:(GADRequestError *)error {
    NSString *adClassName = [ad adNetworkClassName];
    NSString *network = [self getAdNetworkName:adClassName];
    NSString* jsonData = [NSString stringWithFormat:@"{ 'error': '%@', 'network': '%@' }",
                          [error localizedFailureReason],
                          network];

    [self fireEvent:@"" event:@"onFailedToReceiveInterstitialAd" withData:jsonData];
}
...
- (void)interstitialWillPresentScreen:(GADInterstitial *)interstitial {
    ...
    NSString *network = [self getAdNetworkName:adClassName];
    NSString* jsonData = [NSString stringWithFormat:@"{ 'network': '%@' }", network];
    [self fireEvent:@"" event:@"onPresentInterstitialAd" withData:jsonData];
}
...
- (void)interstitialDidDismissScreen:(GADInterstitial *)interstitial {
    ...
    NSString *network = [self getAdNetworkName:adClassName];
    NSString* jsonData = [NSString stringWithFormat:@"{ 'network': '%@' }", network];
    [self fireEvent:@"" event:@"onDismissInterstitialAd" withData:jsonData];
